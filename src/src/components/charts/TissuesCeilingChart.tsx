...
export default function TissuesCeilingChart() {
  ...
  const chartData = useMemo(() => {
    ...
    const data = divePlanner.getTissuesCeilingChartData();
    const x = data.map(d => new Date(1970, 1, 1, 0, 0, d.time, 0));
    const depthY = data.map(d => d.depth);
    ...
    for (let i = 0; i < 16; i++) {
      traces.push({
        x,
        y: data.map(d => d.tissuesCeiling[i]),
        type: 'scatter' as const,
        mode: 'lines' as const,
        name: `Tissue ${i + 1}`,
  ...
  const layout = {
    autosize: true,
    showlegend: false,
    title: { text: 'Tissue Ceilings', y: 0.98 },
    margin: { l: 35, r: 10, b: 30, t: 20, pad: 10 },
    xaxis: { fixedrange: true, tickformat: '%H:%M:%S' },
    yaxis: { fixedrange: true, autorange: 'reversed' as const },
