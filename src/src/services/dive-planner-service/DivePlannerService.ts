...
export class DivePlannerService {
  ...
  settings: DiveSettingsService;

  private diveID = crypto.randomUUID();
  private diveProfile: DiveProfile;

  constructor() {
    this.settings = new DiveSettingsService();
    this.diveSegmentFactory = new DiveSegmentFactoryService(this.settings);
    this.chartGenerator = new ChartGeneratorService(this.diveSegmentFactory, this.settings);
