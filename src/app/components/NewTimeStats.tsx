...
export default function NewTimeStats({ timeAtDepth }: NewTimeStatsProps) {
  ...
  const decoMilestones = getDecoMilestones();
    ...
    <Paper elevation={2} sx={{ p: 2 }}>
      <Box sx={{ display: 'flex', flexDirection: 'column', gap: 1 }}>
        ...
        <Tooltip title="Total dive runtime after the time at depth" placement="right">
          <Typography>
            Total Dive Duration: <strong>{humanDuration(totalDiveDuration)}</strong>
          </Typography>
        </Tooltip>
        {decoMilestones.length > 0 && (
          <>
            <Divider sx={{ my: 1 }} />
            {decoMilestones.map((milestone, index) => (
